...
export default function CmsAdmin({ adminEmail }: CmsAdminProps) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const brandingSnap = await getDoc(brandingDoc);
      ...
      setErrorCode("ဒေတာများ ဆွဲယူရန် မအောင်မြင်ပါ (Failed loading administrative data)");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) { // Increased client-side allow limit, we compress anyway
      alert("ပုံအရွယ်အစား 5MB ထက် မကြီးရပါ (File must be under 5MB)");
      return;
    }
    ...
    try {
      ...
      reader.onloadend = () => {
        ...
        img.onload = async () => {
          ...
          if (!ctx) {
            alert("ဘရောက်ဇာမှ Canvas ကို အထောက်အပံ့မပေးပါ (Canvas not supported)");
          ...
          // Compress to webp if possible, fallback to jpeg/png based on type
          let mimeType = file.type === "image/png" ? "image/png" : "image/webp";
          let quality = 0.8;
          let base64String = canvas.toDataURL(mimeType, quality);

          // If still too big (over ~800KB base64 string), fallback to more compressed JPEG
          if (base64String.length > 800000) {
              base64String = canvas.toDataURL("image/jpeg", 0.6);
          }
          
          if (base64String.length > 1000000) {
             alert("ပုံအရွယ်အစား ပြုပြင်ပြီးသော်လည်း ကြီးလွန်းနေပါသေးသည် (Image still too large after compression)");
             setLogoUploading(false);
             return;
          }
          ...
          try {
            ...
            setLogoSuccessMsg("Logo saved successfully!");
          } catch (uploadErr) {
            console.error("Firestore upload error:", uploadErr);
            alert("သိမ်းဆည်းရာတွင် အမှားအယွင်းဖြစ်ပေါ်ခဲ့ပါသည် (Upload failed)");
          } finally {
            setLogoUploading(false);
          }
        };
        img.onerror = () => {
          alert("ပုံမှားယွင်းနေပါသည် (Invalid image)");
          setLogoUploading(false);
        };
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-3xl p-6 space-y-5">
            <div className="flex items-center gap-2 border-b border-black/5 dark:border-white/5 pb-3">
              ...
              <h2 className="text-xs font-bold uppercase tracking-wider text-black dark:text-slate-100">App Branding Settings</h2>
            </div>
            
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="flex-shrink-0 h-16 w-16 bg-black/10 dark:bg-white/10 border border-black/20 dark:border-white/20 rounded-2xl flex items-center justify-center overflow-hidden">
                  {logoPreview ? (
                    <img src={logoPreview} alt="App Logo" className="h-full w-full object-contain p-2" />
                  ) : (
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest text-center leading-tight">No<br/>Logo</span>
                  )}
                </div>
                <div className="flex-1">
                  ...
                  <label className="flex items-center justify-center w-full gap-2 py-2 px-3 border border-dashed border-black/30 dark:border-white/30 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer transition-colors relative">
                    {logoUploading ? (
                    ...
                    <span className="text-xs font-semibold text-black dark:text-slate-200">
                      {logoUploading ? "Uploading..." : "Select File (Max 2MB)"}
                    </span>
                    <input 
                      type="file" 
                      accept="image/*" 
